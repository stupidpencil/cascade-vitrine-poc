...
<template>
  <UPageSection :title="title" :description="description" orientation="horizontal">
    <UPageGrid class="sm:grid-cols-2">
      ...
      >
        <UIcon :name="symptom.icon" class="size-5 shrink-0 text-muted" />
        <p class="text-sm text-toned">{{ symptom.label }}</p>
      </div>
    </UPageGrid>
  </UPageSection>
</template>
